...
| 値 | キー構成 | 用途 |
|----|----------|------|
| `ip`（デフォルト） | IP アドレスのみ | UA偽装による制限回避を防止。大半の環境で推奨 |
| `ip_ua` | IP + User-Agent ハッシュ | NAT 配下で端末ごとに独立した制限が必要な場合 |
...
不正な値を設定した場合は起動時に警告ログが出力され、自動的に `ip` にフォールバックします。

## API 仕様

API エンドポイントの詳細仕様（統一レスポンス形式・エラーコード一覧・レート制限・リトライ戦略・監視ガイド）は **[docs/api-spec.md](docs/api-spec.md)** を参照してください。
